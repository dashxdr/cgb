...
// **                                                                      **
// ** Last modified : 30 Oct 1997 by John Brandwood                        **
// **                                                                      **
// **************************************************************************
// **************************************************************************
// **************************************************************************
...
static	UD *                Pxl08ToN64              (
								UB *                pu08src,
								UD *                pu32dst,
								UI                  uiwidth);

static	UB *                Buf32ToN64Normal        (
								UD *                pu32src,
								UB *                pu08dst,
								UI                  uiwidth);
...
// *             {                                                          *
// *             UD sprite data in IBM format                               *
// *             }                                                          *
// *           }                                                            *
// **************************************************************************

global	BUFFER *            BitmapToN64Spr          (
								DATABITMAP_T *      pdbiti,
								SI                  sibmx,
								SI                  sibmy,
								UI                  uibmw,
	...
	pcl__Chunk->ud___ckMach = ID4_N64S;
	...
	if (pdbiti->ui___bmB == 8)
		{
		pfuncconv = Pxl08ToN64;
		pu08src   = pdbiti->pub__bmBitmap + (ulwsrc * sibmy) + sibmx;
		}
	else
		{
		sprintf(ErrorMessage,
			"(BitmapToN64Spr) Can't convert %dbpp data.\n",
			pdbiti->ui___bmB);
		goto errorUnknown;
		}

	if (uiSprDirection == BOTTOMTOTOP) ulwsrc = 0 - ulwsrc;

	// Finally, convert the data line-by-line.

	if (uiSprCompression == ENCODED_UNPACKED) {
		pfuncpack = Buf32ToN64Normal;
		} else {
...
global	ERRORCODE           ReformatSprsForN64      (
	...
	if (pub__Buf == NULL)
		...
		sprintf(ErrorMessage,
			"(XVERTN64) Not enough memory to allocate buffer.\n");
		return (ErrorCode);
		}
	...
	if ((pcl__Res != NULL) && (flWriteIDX == YES))
		{
		*pcz__OutputExt = '\0';
	...
	for (ui___i = 0; ui___i < pcl__Spr->ui___sprsCount; ui___i += 1)
		...
		if ((pcl__Idx->si___spriXOffset >  127) ||
			...
			{
			sprintf(ErrorMessage,
				"(XVERTN64) X overflow in sprite 0x%04X during reformat.\n",
		...
		if ((pcl__Idx->si___spriYOffset >  127) ||
			...
			{
			sprintf(ErrorMessage,
				"(XVERTN64) Y overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		if (pcl__Idx->ui___spriWidth  > 255)
			{
			sprintf(ErrorMessage,
				"(XVERTN64) W overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		if (pcl__Idx->ui___spriHeight > 255)
			{
			sprintf(ErrorMessage,
				"(XVERTN64) H overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		...
		if (pcl__Idx->ul___spriBufLen == 0)
			...
			if (pcl__Res != NULL)
				...
				*/
				...
				ui___Tmp =
					((pcl__Idx->si___spriXOffset & 0x00000FFFu) << 20) |
					((pcl__Idx->si___spriYOffset & 0x00000FFFu) <<  8);

				fprintf(pcl__Res, "$%08X!d\n", ui___Tmp);

				if (flWriteRGB == YES)
					{
					*pcz__OutputExt = '\0';

					if (PalInfo.ui___palsCount > 1)
						{
						sprintf(pcz__OutputExt, "_%03d", pcl__Idx->ui___spriPalette - 1);
						}
...
// * PxlToN648x8x4 ()                                                       *
// **************************************************************************
// * Xvert 8bpp bitmap data into an N64 character                           *
// **************************************************************************
...
global	UI                  PxlToN648x8x4           (
	...
	if ((flChrXFlipAllowed == YES) || (flChrYFlipAllowed == YES))
		{
		// Convert the 256 color data into an X-flipped N64 character.
		...
		// Convert the normal N64 character into a Y-flipped character.
		...
		*pu32chr++ = pu32lin[0];
		...
		// Convert the X-flipped N64 character into an X and Y-flipped character.
...
global	UI                  PxlToN648x8x8           (
	...
	// Convert the 256 color data into a normal N64 character.
...
// * Pxl08ToN64 ()                                                          *
// **************************************************************************
// * Xvert one line of 8bpp data into N64 format but padded out to 32bpp    *
// **************************************************************************
...
static	UD *                Pxl08ToN64              (
	...
	if (uiSprCoding == ENCODED_PALETTE)
		...
		if (uiSprBPP == 16)
			{
			sprintf(ErrorMessage,
				"(XVERTN64) Can't convert 8bpp source data into 16bpp palettized data.\n");
			goto errorIllegal;
			}
		...
		sprintf(ErrorMessage,
			"(XVERTN64) Don't know how to convert 8bpp data into RGB data.\n");
		goto errorUnknown;
		}
...
// * Buf32ToN64Normal ()                                                    *
// **************************************************************************
// * Xvert one line of 32bpp padded data into uncompressed N64 format data  *
// **************************************************************************
// * Inputs  UD *            Ptr to source (padded-data) buffer             *
// *         UB *            Ptr to destination buffer                      *
// *         UI              # of pixels to convert                         *
// *                                                                        *
// * Output  UD *            Updated ptr to destination, or NULL if failed  *
// **************************************************************************
...
//	UI                  uipad;
	...
	// Find out how much to pad out the end of line. The N64 requires
...
//		}
	...
	// Flush out the last bits and do not UD align.

	pu08dst = BitIOFlush(pu08dst, FALSE);
