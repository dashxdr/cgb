...
static	UD *                Pxl08ToSAT              (
								UB *                pu08src,
								UD *                pu32dst,
								UI                  uiwidth);

static	UB *                Buf32ToSATNormal        (
								UD *                pu32src,
								UB *                pu08dst,
								UI                  uiwidth);
...
global	BUFFER *            BitmapToSATSpr          (
	...
	if (pdbiti->ui___bmB == 8) {
		pfuncconv = Pxl08ToSAT;
		pu08src   = pdbiti->pub__bmBitmap + (ulwsrc * sibmy) + sibmx;
		}
	else
		{
		sprintf(ErrorMessage,
			"(BitmapToSATSpr) Can't convert %dbpp data.\n",
			pdbiti->ui___bmB);
		goto errorUnknown;
		}
...
global	ERRORCODE           ReformatSprsForSAT      (
	...
	if (pub__Buf == NULL)
		...
		sprintf(ErrorMessage,
			"(XVERTSAT) Not enough memory to allocate buffer.\n");
		return (ErrorCode);
		}
...
//		pcl__FI->ub___fsiH = pcl__Idx->ui___spriHeight - 1;
		...
		if ((pcl__Idx->si___spriXOffset >  127) ||
			...
			{
			sprintf(ErrorMessage,
				"(XVERTSAT) X overflow in sprite 0x%04X during reformat.\n",
		...
		if ((pcl__Idx->si___spriYOffset >  127) ||
			...
			{
			sprintf(ErrorMessage,
				"(XVERTSAT) Y overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		if (pcl__Idx->ui___spriWidth  > 255)
			{
			sprintf(ErrorMessage,
				"(XVERTSAT) W overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		if (pcl__Idx->ui___spriHeight > 255)
			{
			sprintf(ErrorMessage,
				"(XVERTSAT) H overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
...
// * PxlToSAT8x8x4 ()                                                       *
// **************************************************************************
// * Xvert 8bpp bitmap data into an SAT character                           *
// **************************************************************************
...
global	UI                  PxlToSAT8x8x4           (
	...
	if ((flChrXFlipAllowed == YES) || (flChrYFlipAllowed == YES))
		{
		// Convert the 256 color data into an X-flipped Saturn character.
		...
		// Convert the normal Saturn character into a Y-flipped character.
		...
		*pu32chr++ = pu32lin[0];
		...
		// Convert the X-flipped Saturn character into an X and Y-flipped character.
...
global	UI                  PxlToSAT8x8x8           (
	...
	// Convert the 256 color data into a normal Saturn character.
		...
		*pu32idx++ = (UD ) (((UL) ((UB *) pu16dst)) - uloffset);
		...
		while (uij-- != 0)
			...
			if (uiOutputOrder == ORDERSWAP)
				...
				if (uiChrBitSize == 16) {
					while (uiw-- != 0) {
						*(UW *) pu16dst = *(UW *) pu16tmp;
						pu16dst+=sizeof(UW);
						pu16tmp+=sizeof(UW);
						}
					} else {
					while (uiw-- != 0) {
						*(UD *) pu16dst = *(UD *) pu16tmp;
						pu16dst+=sizeof(UD);
						pu16tmp+=sizeof(UD);
						}
					}
...
// * Pxl08ToSAT ()                                                          *
// **************************************************************************
// * Xvert one line of 8bpp data into SAT format but padded out to 32bpp    *
// **************************************************************************
...
static	UD *                Pxl08ToSAT              (
	...
	if (uiSprCoding == ENCODED_PALETTE)
		...
		if (uiSprBPP == 16)
			{
			sprintf(ErrorMessage,
				"(XVERTSAT) Can't convert 8bpp source data into 16bpp palettized data.\n");
			goto errorIllegal;
			}
		...
		sprintf(ErrorMessage,
			"(XVERTSAT) Don't know how to convert 8bpp data into RGB data.\n");
		goto errorUnknown;
		}
...
// * Buf32ToSATNormal ()                                                    *
// **************************************************************************
// * Xvert one line of 32bpp padded data into uncompressed SAT format data  *
// **************************************************************************
...
static	UB *                Buf32ToSATNormal        (
	...
	// Find out how much to pad out the end of line. The Saturn requires
