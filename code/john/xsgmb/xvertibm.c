...
// ** into PC format.                                                      **
// **                                                                      **
// ** Last modified : 14 Nov 1996 by John Brandwood                        **
// **                                                                      **
// **************************************************************************
// **************************************************************************
// **************************************************************************
...
static	UD *                Pxl08ToIBM              (
								UB *                pu08src,
								UD *                pu32dst,
								UI                  uiwidth);

static	UB *                Buf32ToIBMNormal        (
								UD *                pu32src,
								UB *                pu08dst,
								UI                  uiwidth);
...
global	BUFFER *            BitmapToIBMSpr          (
	...
	pcl__Chunk->ud___ckMach = ID4_IbmS;
	...
	if (pdbiti->ui___bmB == 8)
		{
		pfuncconv = Pxl08ToIBM;
		pu08src   = pdbiti->pub__bmBitmap + (ulwsrc * sibmy) + sibmx;
		}
	else
		{
		sprintf(ErrorMessage,
			"(BitmapToIBMSpr) Can't convert %dbpp data.\n",
			pdbiti->ui___bmB);
		goto errorUnknown;
		}
...
global	ERRORCODE           ReformatSprsForIBM      (
	...
	if (pub__Buf == NULL)
		...
		sprintf(ErrorMessage,
			"(XVERTIBM) Not enough memory to allocate buffer.\n");
		return (ErrorCode);
		}
	...
	for (ui___i = 0; ui___i < pcl__Spr->ui___sprsCount; ui___i += 1)
		...
		if ((pcl__Idx->si___spriXOffset >  127) ||
			...
			{
			sprintf(ErrorMessage,
				"(XVERTIBM) X overflow in sprite 0x%04X during reformat.\n",
		...
		if ((pcl__Idx->si___spriYOffset >  127) ||
			...
			{
			sprintf(ErrorMessage,
				"(XVERTIBM) Y overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		if (pcl__Idx->ui___spriWidth  > 255)
			{
			sprintf(ErrorMessage,
				"(XVERTIBM) W overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
		if (pcl__Idx->ui___spriHeight > 255)
			{
			sprintf(ErrorMessage,
				"(XVERTIBM) H overflow in sprite 0x%04X during reformat.\n",
				(pcl__Idx->si___spriNumber));
			}
...
// * PxlToIBM8x8x4 ()                                                       *
// **************************************************************************
// * Xvert 8bpp bitmap data into an IBM character                           *
// **************************************************************************
...
global	UI                  PxlToIBM8x8x4           (
	...
	if ((flChrXFlipAllowed == YES) || (flChrYFlipAllowed == YES))
		{
		// Convert the 256 color data into an X-flipped IBM character.
		...
		// Convert the normal IBM character into a Y-flipped character.
		...
		*pu32chr++ = pu32lin[0];
		...
		// Convert the X-flipped IBM character into an X and Y-flipped character.
...
global	UI                  PxlToIBM8x8x8           (
	...
	// Convert the 256 color data into a normal IBM character.
...
// * Pxl08ToIBM ()                                                          *
// **************************************************************************
// * Xvert one line of 8bpp data into IBM format but padded out to 32bpp    *
// **************************************************************************
...
static	UD *                Pxl08ToIBM              (
	...
	if (uiSprCoding == ENCODED_PALETTE)
		...
		if (uiSprBPP == 16)
			{
			sprintf(ErrorMessage,
				"(XVERTIBM) Can't convert 8bpp source data into 16bpp palettized data.\n");
			goto errorIllegal;
			}
		...
		sprintf(ErrorMessage,
			"(XVERTIBM) Don't know how to convert 8bpp data into RGB data.\n");
		goto errorUnknown;
		}
...
// * Buf32ToIBMNormal ()                                                    *
// **************************************************************************
// * Xvert one line of 32bpp padded data into uncompressed IBM format data  *
// **************************************************************************
...
//	UI                  uipad;
	...
	// Find out how much to pad out the end of line. The IBM requires
